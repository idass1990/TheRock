...
test_matrix = {
    ...
    # cmake/build-essential/libdrm-dev to build rocjitsu). gfx950's presubmit entry sets
    # submodule_bump_tests_only, which would otherwise skip this component entirely on ordinary
    # PRs -- it still runs there because linux_cpu_runner components bypass that gate (see
    # CPU_ONLY_TEST_RUNS_ON in configure_multi_arch_ci.py).
    "miopen-dbsync": {
